 ...
 */
public class PuppetClusterBidderSellerCombined implements PuppetI {

	private static final Logger logger = Logger.getLogger(PuppetClusterBidderSellerCombined.class); 
	
	private final ClusterBidder normal;
	private final TMSeller tmSeller;
	private final Puppet puppetV;

	private final Controller controller;
	private final BufferHolder bh;
	
	public PuppetClusterBidderSellerCombined(BufferHolder bh, PaymentSender ps, ItemSender is, AuctionHouse ah, List<ItemType> itemTypes, final Controller controller) {
		...
		if (Math.random() < 0.5) {
			this.normal = new ClusterEarly(bh, ps, is, ah, itemTypes) {
				protected int numberOfAuctionsPer100Days(double random) { // modify behaviour to participate make it participate in more auctions...
					return super.numberOfAuctionsPer100Days(r.nextDouble());
				}
			};
		} else {
...
//		System.out.println(normal.getId() + ", " + normal.interestTimes);
		...
		this.tmSeller = new TMSeller(bh, ps, is, ah, itemTypes, normalId);
	...
	public static PuppetFactoryI getFactory() {
		return new PuppetFactoryI() {
			...
			public PuppetI instance(BufferHolder bh, PaymentSender ps, ItemSender is, AuctionHouse ah, Controller controller, List<ItemType> itemTypes) {
				return new PuppetClusterBidderSellerCombined(bh, ps, is, ah, itemTypes, controller);
			}
		};
	}
